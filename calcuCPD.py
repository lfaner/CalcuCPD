...
import os
from calculos import calcular_neto_cheque
...
def _parse_excel_cheques(file_storage) -> list[dict]:
    ...
    if not form_rows:
        raise ValueError("No se encontraron cheques en el Excel")
    ...
    if request.method == "POST":
        try:
            ...
            if not valores or not vencimientos or not tasas:
                raise ValueError("Debe cargar al menos un cheque completo")
...
def cargar_excel():
    ...
    if not file or file.filename == "":
        ...
        try:
            form_rows = _parse_excel_cheques(file)
            try:
                next_row_id = max(int(r["row_id"]) for r in form_rows) + 1
            except ValueError:
                next_row_id = len(form_rows) + 1
...
def modelo_excel():
    return send_from_directory("static", "modelo_cheques.xlsx", as_attachment=True)
